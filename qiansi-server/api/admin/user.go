...
func UserSigin(c *gin.Context) {
	...
	models.NewApiResult(c, 1, "登陆成功", u).Json(c)
}
...
func UserSiginUp(c *gin.Context) {
	...
	if len(password) < 6 {
		models.NewApiResult(c, -4, "密码格式不正确", nil).Json(c)
		return
	}
	...
	if member.Uid > 0 {
		models.NewApiResult(c, 1, "注册成功", u).Json(c)
		return
	}
	models.NewApiResult(c, 0, "注册失败", u).Json(c)
}
